...
- (void)panToBack:(UIPanGestureRecognizer*)pan
{
    ...
    else if(xoffset<0) //向左滑
    {
        ...
        {
            xoffset = xoffset<-SCREEN_WIDTH?-SCREEN_WIDTH:xoffset;
        }
        else
        {
            xoffset = 0;
...
- (void)showPreViewController
{
    ...
    if (count>1)
    {
        ...
        animatedTime = ABS(SCREEN_WIDTH - currentView.frame.origin.x) / SCREEN_WIDTH * 0.35;
        [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
        [UIView animateWithDuration:animatedTime animations:^{
            [self layoutCurrentViewWithOffset:UIOffsetMake(SCREEN_WIDTH, 0)];
...
- (void)layoutCurrentViewWithOffset:(UIOffset)offset
{
    ...
    if (count>1)
    {
        UIViewController *currentVC = [self topViewController];
        UIViewController *preVC = [self.viewControllers objectAtIndex:count-2];
        [currentVC.view setFrame:CGRectMake(offset.horizontal, 0, SCREEN_WIDTH, self.view.frame.size.height)];
        [preVC.view setFrame:CGRectMake(offset.horizontal/2-self.view.frame.size.width/2, 0, SCREEN_WIDTH, self.view.frame.size.height)];
    }
}
