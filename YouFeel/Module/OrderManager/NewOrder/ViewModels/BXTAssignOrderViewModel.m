...
@implementation BXTAssignOrderViewModel
...
- (instancetype)init
{
    if (self = [super init])
    {
        ...
        self.orderID = [[BXTGlobal shareGlobal].assignOrderIDs objectAtIndex:index - 1];
        [self initialBind];
    }
    return self;
}
...
- (void)initialBind
{
    ...
    RACCommand *acceptCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
        ...
        return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
            @strongify(self);
            BXTDataRequest *request = [[BXTDataRequest alloc] init];
            [request reaciveDispatchedOrderID:self.orderID success:^(RequestType type, id response) {
                NSDictionary *dic = (NSDictionary *)response;
    ...
    [[acceptCommand.executing skip:1] subscribeNext:^(id x) {
        if ([x boolValue] == YES)
        {
            // 正在执行
            [BXTGlobal showLoadingMBP:@"加载中..."];
    ...
    self.acceptOrderCommand = acceptCommand;
    
    // 我不接
    RACCommand *rejectCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
        
        UINavigationController *nav = input;
        
        return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
            @strongify(self);
            BXTRejectOrderViewController *rejectVC = [[BXTRejectOrderViewController alloc] initWithOrderID:self.orderID viewControllerType:AssignVCType];
            [nav pushViewController:rejectVC animated:YES];
    ...
    self.rejectOrderCommand = rejectCommand;
}
